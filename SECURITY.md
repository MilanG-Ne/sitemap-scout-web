...
- Apache/LiteSpeed receives an 8 KiB request-body cap plus explicit rewrite rules for oversized declared Content-Length and unsupported API methods. Whether the size rule runs before PHP depends on the host exposing that header to rewrite rules. PHP independently bounds body reads for requests that reach it. Enforcement of streaming/chunked input limits depends on the hosting server. These controls do not replace a front-end request-rate limiter or DDoS protection.
